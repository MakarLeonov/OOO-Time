...
export default {
    ...
    methods: {
        addEntry() {
            this.v$.$validate();
            if (!this.v$.$error) {
                let url = `http://127.0.0.1:8000/api/service/${this.item.id}?name=${this.title}&cost=${this.cost}&is_popular=${this.is_popular}&repair_types_id=${this.repair_types_id}&description=${this.description}`;
                this.$store.commit('EditServiceModalWindow');
                this.$store.dispatch('EDIT_SERVICE_ENTRY', url);
            }
        },
