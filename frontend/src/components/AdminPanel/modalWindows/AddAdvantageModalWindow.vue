    ...
    <div class="modal" @keyup.ctrl.enter="addEntry()">
        <div class="modal-window">
            <div class="modal-window-head">
                ...
                <div class="close" @click="this.$store.commit('AddAdvantageModalWindow')">
...
export default {
    ...
    methods: {
        addEntry() {
            this.v$.$validate();
            if (!this.v$.$error) {
                let url = `http://127.0.0.1:8000/api/advantages?title=${this.title}&description=${this.description}`;
                this.$store.commit('AddAdvantageModalWindow');
                this.$store.dispatch('ADD_ADVANTAGE', url);
            } else {
                console.log('error')
            }
        },
