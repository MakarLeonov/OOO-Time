    ...
    <div class="modal" @keyup.ctrl.enter="editEntry()">
        <div class="modal-window">
            <div class="modal-window-head">
                <p class="title">Таблица "Преимущества"</p>
                <div class="close"  @click="this.$store.commit('EditAdvantegeModalWindow')">
...
export default {
    ...
    methods: {
        editEntry() {
            this.v$.$validate();
            if (!this.v$.$error) {
                let url = `http://127.0.0.1:8000/api/advantages/${this.item.id}?title=${this.title}&description=${this.description}`;
                this.$store.commit('EditAdvantegeModalWindow');
                this.$store.dispatch('EDIT_ADVANTAGE', url);
            } else {
                console.log('error')
            }
        }
