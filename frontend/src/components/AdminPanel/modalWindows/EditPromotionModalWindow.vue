<template>
    <div class="modal" @keyup.ctrl.enter="editEntry()">
        <div class="modal-window">
            <div class="modal-window-head">
                <p class="title">Таблица "Акции"</p>
                <div class="close"  @click="this.$store.commit('EditPromotionModalWindow')">
            ...
            <div class="modal-window-body">
                ...
                <my-button @click="editEntry()" class="button">Редактировать запись</my-button>
            </div>
        </div> 
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            beginning: this.item.beginning,
            ending: this.item.ending,
        }
    },
    ...
    methods: {
        editEntry() {
            this.v$.$validate();
            if (!this.v$.$error) {
                let url = `http://127.0.0.1:8000/api/promotions/${this.item.id}?title=${this.title}&description=${this.description}&beginning=${this.beginning}&ending=${this.ending}`;
                this.$store.commit('EditPromotionModalWindow');
                this.$store.dispatch('EDIT_PROMOTION', url);
            } else {
                console.log('error')
            }
        }
